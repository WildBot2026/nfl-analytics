...
# REAL NFL 2026 Schedule: [week, away, home, note]
schedule_data = {
...
week_labels = {
    1: "Semana 1 (Sep 9-14)", 2: "Semana 2 (Sep 17-21)", 3: "Semana 3 (Sep 24-28)",
    4: "Semana 4 (Oct 1-5)", 5: "Semana 5 (Oct 8-12)", 6: "Semana 6 (Oct 15-19)",
    7: "Semana 7 (Oct 22-26)", 8: "Semana 8 (Oct 29-Nov 2)", 9: "Semana 9 (Nov 5-9)",
    10: "Semana 10 (Nov 12-16)", 11: "Semana 11 (Nov 19-23)", 12: "Semana 12 (Nov 25-29)",
    13: "Semana 13 (Dec 3-7)", 14: "Semana 14 (Dec 10-14)", 15: "Semana 15 (Dec 17-21)",
    16: "Semana 16 (Dec 24-28)", 17: "Semana 17 (Dec 31-Ene 4)", 18: "Semana 18 (Ene 10)",
}

def calc_spread(away, home):
    av = st.get(away, 70)
    hv = st.get(home, 70)
    sp = round((hv - av) / 5 * 3, 1)
    if abs(sp) < 0.3:
        sp = 0.0
...
def calc_ou(away, home):
    av = st.get(away, 70)
    hv = st.get(home, 70)
    return round(35 + (av + hv) / 6, 1)

def parse_game(away, home, note):
    ...
    sp = calc_spread(away, home)
    ou = calc_ou(away, home)
    return [away, home, sp, ou, dt, special]

games = []
team_counts = {t: 0 for t in tc}
for wk in sorted(schedule_data):
    for away, home, note in schedule_data[wk]:
        g = parse_game(away, home, note)
        games.append([wk] + g)
        team_counts[away] += 1
        team_counts[home] += 1

print("=== Game Counts ===", file=sys.stderr)
all_ok = True
for t, c in sorted(team_counts.items()):
    ok = "✓" if c == 17 else f"✗({c})"
    if c != 17: all_ok = False
    print(f"  {t}: {c} {ok}", file=sys.stderr)
print(f"Total: {len(games)}", file=sys.stderr)
assert len(games) == 272, f"Expected 272, got {len(games)}"
assert all_ok, "Some teams != 17 games"

# Build JS games array
ga_entries = []
for g in games:
    w,a,h,s,o,d,n = g
...
# Week options
wl_options = "\n".join(f'<option value="{wk}">{week_labels[wk]}</option>' for wk in sorted(week_labels))

# Escape special chars for embedding in JS
...
# Need to be careful with JS template literal syntax vs Python f-strings
# We'll use Python string replace
...
function sg(w,a,h){
  ...
  el.innerHTML='<div class="dt-inner"><div class="hb"><div></div><button onclick="document.getElementById(\'dt\').className=\'\'